...
1. **Bump the version** in both `pyproject.toml` (`[project].version`) and
   `src/loopllm/__init__.py` (`__version__`) — they must match exactly, or
...
3. **Add the `PYPI_API_TOKEN` repo secret** (Settings -> Secrets and
   ...
   `azank1/loop-llm`** — `gh secret list` returns empty — so the workflow
   ...
   `loopllm` project (or your PyPI account, for the very first upload before
...
5. **Verify**: `pip install "loopllm[mcp]==X.Y.Z"` in a scratch venv and
   confirm `python -c "import loopllm; print(loopllm.__version__)"` matches.
...
4. **Get a publisher access token**: create/verify the `loopllm` publisher
...
5. **Publish**:
   ...
   ```
   (or `vsce login loopllm` once, then `vsce publish` without `-p` each time).
