"""Tests for per-project state scoping (~/.cdv/projects/<id>/)."""
...
def test_env_override_wins_regardless_of_cwd(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CDV_PROJECT", "my-explicit-project")
    ...
    monkeypatch.delenv("CDV_PROJECT")
    # Once the override is gone, the repos diverge based on their own remotes.
    assert resolve_project_id(repo_a) != resolve_project_id(repo_b)


def test_workspace_folder_paths_beats_home_cwd(
    tmp_path: Path, monkeypatch
) -> None:
    """Cursor starts MCP with cwd=$HOME but sets WORKSPACE_FOLDER_PATHS.

    Without this, Loop Monitor (cdv paths from the workspace) and the MCP
    sidecar write to different ~/.cdv/projects/<id>/ trees.
    """
    monkeypatch.delenv("CDV_PROJECT", raising=False)
    monkeypatch.delenv("CDV_WORKSPACE", raising=False)
    remote = "https://github.com/acme/widgets.git"
    workspace = _init_repo(tmp_path, "workspace", remote=remote)
    homeish = tmp_path / "fake_home"
    homeish.mkdir()

    monkeypatch.setenv("WORKSPACE_FOLDER_PATHS", str(workspace))
    monkeypatch.chdir(homeish)

    assert resolve_workdir() == workspace.resolve()
    assert resolve_project_id() == resolve_project_id(workspace)
    assert resolve_project_id() != resolve_project_id(homeish)


def test_explicit_cwd_beats_workspace_folder_paths(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.delenv("CDV_PROJECT", raising=False)
    monkeypatch.delenv("CDV_WORKSPACE", raising=False)
    repo_a = _init_repo(tmp_path, "a", remote="https://github.com/acme/a.git")
    repo_b = _init_repo(tmp_path, "b", remote="https://github.com/acme/b.git")
    monkeypatch.setenv("WORKSPACE_FOLDER_PATHS", str(repo_a))
    assert resolve_project_id(repo_b) == resolve_project_id(repo_b)
    assert resolve_project_id(repo_b) != resolve_project_id(repo_a)


def test_cdv_workspace_env_beats_workspace_folder_paths(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.delenv("CDV_PROJECT", raising=False)
    preferred = _init_repo(
        tmp_path, "preferred", remote="https://github.com/acme/preferred.git"
    )
    other = _init_repo(tmp_path, "other", remote="https://github.com/acme/other.git")
    monkeypatch.setenv("WORKSPACE_FOLDER_PATHS", str(other))
    monkeypatch.setenv("CDV_WORKSPACE", str(preferred))
    monkeypatch.chdir(tmp_path)
    assert resolve_workdir() == preferred.resolve()
    assert resolve_project_id() == resolve_project_id(preferred)
...
def test_project_state_dir_layout(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CDV_PROJECT", "acme-widgets")
    base = tmp_path / ".cdv"
